...
if __name__ == '__main__':
    ...
    TRAIN_DATASET = FastDataLoader(dataset=opt.dataset, root=opt.data_path, split='train')
    TEST_DATASET = FastDataLoader(dataset=opt.dataset, root=opt.data_path, split='test')
    ...
    for epoch in range(0, opt.nepoch):
        ...
        for batch_id, data in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            pts, gt_cls, gt_rot_bin, gt_Rmat_noi = data
            pts, gt_cls, gt_rot_bin, gt_Rmat_noi = \
